...
import logo from "../../assets/imgs/FinX_lg.svg"
...
export default function CompanySelect() {
  ...
  const navigate = useNavigate();
   ...
   let Company =  JSON.parse(localStorage.getItem("CompanyMaster"));
  ...
  const companyselectsubmit = async (e) => {
         
    if (CompanyId==0) {
      setLoginStatus("Failed");         
      ShowAlert("Error", "Select Company");
      return;
    }
      ...
      const frmData = {txtUsername:Username,txtPassword:Password,txtBRUMapId:"0",txtCompanyId:CompanyId}
                  ...
                  <div className="card-body">
                    ...
                    <div className="pt-4 pb-2">
                      <h5 className="card-title text-center pb-0 fs-4">Login to Your Account</h5>
                        ...
                        <form onSubmit={companyselectsubmit}>
                          ...
                          <select  className="form-control"  onChange= {e => onChange(e.target.value)}> 
                                            ...
                                            {  //Combo Data binding                                                        
                                                        Company.map((res) => 
                                                        (<option key={res.CompanyId} value={res.CompanyId}>{res.Name}</option>))
                                            }
                                        </select>
                          </div>
